...
- 牌背与公共 Token 图片可公开缓存；卡面图片每次都由当前会话和牌的即时状态鉴权，并禁用缓存。别人的私有手牌、盖住的公共牌与牌叠中的牌都不能读取正面图片。

## 资源而不是规则

JSON 包只负责“桌上有什么”，不负责“应该怎么玩”。牌可以自由移动、翻面、旋转、转交或叠成牌堆；骰子、计数器和行动玩家也只是独立工具。UNO 包不会自动判断颜色是否合法，扑克包不会自动比较牌型，这些都由同桌玩家自行约定。

两张以上公共牌明显重叠时，桌面会将它们识别成牌堆。抽牌、展开和洗牌是房主进程中的原子操作；洗牌还会更换资源实例 ID，避免玩家用旧 ID 追踪已经看过的牌。
