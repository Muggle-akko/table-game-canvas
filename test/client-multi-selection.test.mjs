...
async function fixture(t) {
  ...
  const f = { room, link, guestLink, indexedDB, storage, clients: [], hold: false, held: [] };
  const fetch = async (url, options = {}) => {
    const message = options.body ? JSON.parse(options.body).message : null;
    if (f.hold && message?.type === "command" && message.command.type === "move-resources") await new Promise((resolve) => f.held.push(resolve));
