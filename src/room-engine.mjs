...
export function publicStackForCard(room, cardId) {
...
export function tokenStackIndex(tokens) {
...
export function validatePortablePack(pack) {
...
export function replaceRoomPack(room, playerId, pack, { randomizeDeck = true } = {}) {
  const actor = requirePlayer(room, playerId);
  if (actor.role !== "host") {
...
export function playerForSession(room, sessionToken) {
  const playerId = room.sessions.get(sessionToken);
  return playerId ? room.players.get(playerId) ?? null : null;
}

export function setPlayerConnection(room, playerId, delta) {
  const player = requirePlayer(room, playerId);
...
export function addRoomPack(room, playerId, pack, position = {}) {
  const actor = requirePlayer(room, playerId);
  validateGamePack(pack);
...
export function cardDealPlan(available, recipients, { mode = "count", count = 1, remainder = "keep" } = {}) {
...
export function exportRoomScene(room, playerId, name = room.title) {
...
export function validateRoomScene(scene) {
...
export function restoreRoomScene(room, playerId, scene) {
  const actor = requirePlayer(room, playerId);
